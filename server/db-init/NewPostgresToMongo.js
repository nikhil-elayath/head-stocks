...
async function insertCompanies() {
  ...
  for (ticker of tickerNames) {
    ...
    let indicator_date = [];
    const indicatorDates = `select distinct(dates) from simfin where ticker = '${ticker}' group by dates order by dates`;
      ...
      if (f_res.length > 0) {
        f_res.forEach(f_result => {
          if (f_result.hasOwnProperty("indicatorvalue")) {
            obj[`${f_result["indicatorname"].replace(/\./g, "")}`] = parseFloat(
              f_result["indicatorvalue"]
            );
          }
        });
